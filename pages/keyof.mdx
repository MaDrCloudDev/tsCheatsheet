...
`keyof` is a keyword in TypeScript used to extract the key type from an object type. when used on an object type with explicit keys, `keyof` creates a union type with them
